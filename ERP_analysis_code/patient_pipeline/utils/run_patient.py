...
# Window v1
def run_patient_online_sessions_window_v1(patient, last_session_nr, calibration_selection):
    ...
    window_result = online_window_simulation_v1(data_train, data_test, features_train, features_test, log_process=f"p{patient}_online_window_s{first_online}_v1.log")
    performances.update({f"p{patient}_window_v1_s{first_online}":window_result})
    ...
    for i in range(first_online+1,last_session_nr):
        ...
        # 3. Online simulation transfer fixed (trained on session i-1 - applied on session i)
        window_result = online_window_simulation_v1(data_train, data_test, features_train, features_test, log_process=f"p{patient}_online_window_s{i}_v1.log", title_text=plot_title_text)

        performances.update({f"p{patient}_window_v1_s{i}":window_result})
